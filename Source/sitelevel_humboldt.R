...
denbins <- function(curdat,curname){
  sapply(fclimnames,function(x){
    splotbyclim(curdat=curdat,curname=curname,climname=x)
    })
...
tiff(filename=paste0("Outputs/humboldt_data_illustration_",format(Sys.Date(),"%d%b%Y"),".tiff"),
  width=8.5, height=3.5, units="in", res=600)

par(mfrow=c(1,3),mar=c(2,2,2,2)+0.1,oma=c(3,3,3,3))
bluedat <- splotbyclim(spl[[20]],names(spl)[20],climname="pretemp8",incldat=T,thomas=F,
  axlabs=F,ylim=yrange)
...
brimdat <- splotbyclim(spl[[2]],names(spl)[2],climname="pretemp8",incldat=T,thomas=F,
  axlabs=F,xlim=xrange,ylim=yrange)
mtext(expression(bold((b))~italic(Gonepteryx~rhamni)),side=3,line=0.5,xpd=T,adj=0.05,cex=0.9)

addxlab("Population density")

plot(1,1,type="n",xlab="",ylab="n",xaxt="n",yaxt="n",bty="n",ann=F)
legend("left",pch=c(16,21),col=c("blue","red"),
  legend=c("cool (<17°C)","warm (>17°C)"),
  title="August\ntemperature",bty="n",cex=1.15)
