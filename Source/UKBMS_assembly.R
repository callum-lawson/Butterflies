### Assemble UKBMS data and combine with site and climate variables ###
...
### UKBMS
colform <- c("factor","factor","factor","integer","integer")
fullind <- read.csv("Data/ukbms_siteindex.csv",header=T,colClasses=colform)
names(fullind) <- tolower(names(fullind))
names(fullind)[names(fullind)=="sindex"] <- "count"

ind <- subset(fullind,count>=0 & year>=1976) # gets rid of other count codes
...
sitevis <- sumframe(indm)
  # sums counts over all broods for that year (?)

### Read in species traits

sps <- read.csv("Data/2013/speciestraits208.csv",header=T) 
  # Roger Dennis species data
spi <- read.csv("Data/ukbms_species.csv",header=T) # BMS species index
names(spi) <- tolower(names(spi))
spi$species <- as.factor(spi$species)
...
splotbyclim <- function(curdat,curname,climname,nclim=2,ndens=10,incldat=F,thomas=T,axlabs=F,...){
  ...
  if(thomas==T){
    ...
    thom.row <- match(curname,thom[,colnames(thom)=="common_name"])
...
spi$sci.name[spi$common_name==names(spl)[13]] # temp 4
spi$sci.name[spi$common_name==names(spl)[45]] # temp 1
spi$sci.name[spi$common_name==names(spl)[29]] # temp 2
...
rownames(broodmat) <- spi$common_name[match(as.numeric(rownames(broodmat)),spi$BMScode)]
