...
#setwd("D:/Users/calluml/Dropbox/PhD/Analyses/BMS")
...
source("Code/data_processing_functions_28Apr2013.R") # Change to most recent version
...
mod1a <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ (1|site),
	family="poisson",data=curdat)

mod1b <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ (1|year),
	family="poisson",data=curdat)

mod1c <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ (1|site) + (1|year),
	family="poisson",data=curdat)

mod1d <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens)
	+ lpredens 
	+ (1|site) + (1|year),
	family="poisson",data=curdat)

mod1e <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens)
	+ lpredens 
	+ (1+lpredens|site) + (1|year),
	family="poisson",data=curdat)

mod1f <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens)
	+ lpredens 
	+ (1|site) + (1+lpredens|year),
	family="poisson",data=curdat)

mod1g <- lmer(count ~ offset(log(tlen))
	+ offset(lpredens) 
	+ lpredens 
...
mod2a <- lmer(count ~ offset(log(tlen))
 	+ offset(lpredens) 
	+ lpredens 
	+ temp8
	+ (1+lpredens|site),
...
mod2b <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ lpredens 
	+ temp8
	+ lpredens:temp8
...
mod2c <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ lpredens 
	+ pretemp8
	+ (1+lpredens|site),
...
mod2d <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ lpredens 
	+ pretemp8
	+ lpredens:pretemp8
...
mod2e <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ lpredens 
	+ temp8
	+ pretemp8
...
mod2g <- lmer(count ~ offset(log(tlen)) 
	+ offset(lpredens) 
	+ lpredens 
	+ temp8
	+ pretemp8
...
mm1A <- model.matrix(~lpdseq*rep(mean(pretemp8seq),n.seq))
plambda1A <- matrix(mm1A %*% params,nc=1)
mm1B <- model.matrix(~rep(mean(lpdseq),n.seq)*pretemp8seq)
plambda1B <- matrix(mm1B %*% params,nc=1)

par(mfrow=c(1,2))
plot(plambda1A ~ exp(lpdseq),type="l")
with(curdat, points(log(growth)~exp(lpredens)))
plot(plambda1B ~ pretemp8seq,type="l")
with(curdat, points(log(growth)~pretemp8))

mm2 <- model.matrix(~lpdseq2*pretemp8seq2)
plambda2 <- matrix(mm2 %*% params,nc=n.seq)
par(mfrow=c(1,1))
persp(exp(lpdseq),pretemp8seq,plambda2,
	theta=45,phi=15,
	xlab=expression(Density (nm^-2)),
	ylab="Mean Temperature",
	zlab="log(lambda)",
...
require("rgl")
plot3d(pretemp8seq2,exp(lpdseq2),plambda3)
